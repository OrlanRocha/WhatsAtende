...
class EvolutionService
{
    ...
    public function fetchInstances(?string $instanceName = null): array
    {
        ...
        $query = [];
        if (is_string($instanceName) && trim($instanceName) !== '') {
            $query['instanceName'] = trim($instanceName);
        }

        return $this->makeRequest($config, 'GET', '/instance/fetchInstances', $query !== [] ? $query : null);
    }
    ...
    public function sendMessage(array $payload): array
    {
        ...
        $normalized = $this->prepareSendPayload($payload);

        return $this->makeRequest($config, 'POST', '/message/sendText/' . rawurlencode($config['instance']), $normalized);
    }
    ...
    public function sendText(string $contactExternalId, string $message, array $options = []): array
    {
        ...
        foreach (['delay', 'linkPreview', 'mentionsEveryOne', 'quoted'] as $key) {
            if (array_key_exists($key, $options)) {
                $payload[$key] = $options[$key];
            }
        }

        if (!isset($payload['delay']) && isset($options['delay'])) {
     ...
     */
    private function prepareSendPayload(array $payload): array
    {
        if (!isset($payload['number'])) {
            throw new RuntimeException('Payload de envio deve conter o número do destinatário.');
        }

        $number = $this->sanitizeContactNumber((string) $payload['number']);
        $text = isset($payload['text']) ? $this->sanitizeMessageBody((string) $payload['text']) : '';

        if ($text === '') {
            throw new RuntimeException('Payload de envio deve conter uma mensagem.');
        }

        $options = $this->normalizeSendOptions($payload);
        ...
        if (isset($options['delay'])) {
            $normalized['delay'] = max(0, (int) $options['delay']);
        }
        if (isset($options['linkPreview'])) {
            $normalized['linkPreview'] = (bool) $options['linkPreview'];
        }
        if (isset($options['mentionsEveryOne'])) {
            $normalized['mentionsEveryOne'] = (bool) $options['mentionsEveryOne'];
        }

        $quoted = $options['quoted'] ?? ($payload['quoted'] ?? null);
        if (is_array($quoted)) {
            $sanitizedQuoted = $this->sanitizeQuotedPayload($quoted);
            if ($sanitizedQuoted !== []) {
                $normalized['quoted'] = $sanitizedQuoted;
            }
        }
     ...
     * @return array<string, mixed>
     */
    private function normalizeSendOptions(array $payload): array
    {
        $options = [];
    ...
    /**
     * @param array<string, mixed> $quoted
     * @return array<string, mixed>
     */
    private function sanitizeQuotedPayload(array $quoted): array
    {
        $result = [];

        if (isset($quoted['key']) && is_array($quoted['key'])) {
            $key = [];
            if (isset($quoted['key']['remoteJid'])) {
                $key['remoteJid'] = (string) $quoted['key']['remoteJid'];
            }
            if (isset($quoted['key']['fromMe'])) {
                $key['fromMe'] = (bool) $quoted['key']['fromMe'];
            }
            if (isset($quoted['key']['id'])) {
                $key['id'] = (string) $quoted['key']['id'];
            }

            if (isset($key['remoteJid'], $key['id'])) {
                $result['key'] = $key;
            }
        }

        if (isset($quoted['message']) && is_array($quoted['message'])) {
            $result['message'] = $this->truncateResponse($quoted['message']);
        }
