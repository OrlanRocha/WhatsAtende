...
class EvolutionController
{
    ...
    public function send(): void
    {
        ...
        foreach (['delay', 'linkPreview', 'mentionsEveryOne'] as $optionKey) {
            if (array_key_exists($optionKey, $payload)) {
                $messagePayload[$optionKey] = $payload[$optionKey];
            }
        }

        if (isset($payload['options']) && is_array($payload['options'])) {
            foreach (['delay', 'linkPreview', 'mentionsEveryOne'] as $optionKey) {
                if (!array_key_exists($optionKey, $messagePayload) && array_key_exists($optionKey, $payload['options'])) {
                    $messagePayload[$optionKey] = $payload['options'][$optionKey];
                }
            }
        }

        $quoted = null;
        if (isset($payload['quoted']) && is_array($payload['quoted'])) {
            $quoted = $payload['quoted'];
        } elseif (isset($payload['options']) && is_array($payload['options']) && isset($payload['options']['quoted']) && is_array($payload['options']['quoted'])) {
            $quoted = $payload['options']['quoted'];
        }

        if ($quoted !== null) {
            $messagePayload['quoted'] = $quoted;
        }

        $result = $this->evolution->sendMessage($messagePayload);
    ...
    public function info(): void
    {
        ...
        $result = $this->evolution->getApiInformation();
        $this->respondJson($result);
    }
    ...
    public function instances(): void
    {
        ...
        $instanceName = trim((string) ($_GET['instanceName'] ?? ''));
        $result = $this->evolution->fetchInstances($instanceName !== '' ? $instanceName : null);
        $this->respondJson($result);
    }
    ...
    /**
     * @param array{status:int, success?:bool, data:mixed, error:?string, error_detail?:?string} $result
     */
