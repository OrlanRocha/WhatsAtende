...
- **Evolution API Nativa:** Endpoint autenticado em `/api/evolution` expõe ações de chats, mensagens, mídias, informações do servidor (`/api/evolution/info`) e inventário de instâncias (`/api/evolution/instances`) utilizando API Key/token definidos pelo administrador. O painel administrativo exibe a versão do servidor Evolution e o status da instância configurada.
